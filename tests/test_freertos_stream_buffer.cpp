 ...
 * stream buffer wrapper library, including static and dynamic stream buffer
 * allocators with comprehensive stream buffer operations testing.
 *
 * Test Coverage:
...
TEST_F(FreeRTOSStreamBufferTest,
       StaticStreamBufferAllocatorCreateWithTriggerLevel) {
...
TEST_F(FreeRTOSStreamBufferTest,
       DynamicStreamBufferAllocatorCreateWithTriggerLevel) {
...
TEST_F(FreeRTOSStreamBufferTest, DynamicStreamBufferAllocatorCreateFailure) {
  ...
  EXPECT_CALL(*mock, xStreamBufferCreate(2048, 1)).WillOnce(Return(nullptr));
...
TEST_F(FreeRTOSStreamBufferTest, StreamBufferDestructionNullHandle) {
  EXPECT_CALL(*mock, xStreamBufferCreate(150, 1)).WillOnce(Return(nullptr));
...
TEST_F(FreeRTOSStreamBufferTest, StreamBufferCreationFailure) {
  ...
  // Note: In the actual implementation, configASSERT would trigger when
  // creation fails This test documents the expected behavior but cannot be
  // fully tested in host environment due to assertion handling. In real
  // FreeRTOS, assertion failure would halt execution.
...
TEST_F(FreeRTOSStreamBufferTest, StaticVsDynamicBehavior) {
  ...
  // Static buffer test
  EXPECT_CALL(*mock, xStreamBufferCreateStatic(BUFFER_SIZE, TRIGGER_LEVEL,
                                               NotNull(), NotNull()))
      .WillOnce(Return(mock_stream_buffer_handle));
  EXPECT_CALL(*mock, xStreamBufferSend(mock_stream_buffer_handle, NotNull(), 10,
                                       portMAX_DELAY))
...
// =============================================================================
// Branch Coverage Tests - ISR no-arg, iterator ISR, and additional paths
// =============================================================================

TEST_F(FreeRTOSStreamBufferTest, StreamBufferSendIsrNoArg_BranchCoverage) {
  EXPECT_CALL(*mock, xStreamBufferCreate(128, 1))
      .WillOnce(Return(mock_stream_buffer_handle));
  EXPECT_CALL(*mock, xStreamBufferSendFromISR(mock_stream_buffer_handle,
                                              NotNull(), 4, NotNull()))
...
TEST_F(FreeRTOSStreamBufferTest,
       StreamBufferSendIsrIteratorsNoWoken_BranchCoverage) {
  EXPECT_CALL(*mock, xStreamBufferCreate(512, 1))
      .WillOnce(Return(mock_stream_buffer_handle));
  EXPECT_CALL(*mock, xStreamBufferSendFromISR(mock_stream_buffer_handle,
                                              NotNull(), 5, NotNull()))
...
TEST_F(FreeRTOSStreamBufferTest,
       StreamBufferSendChronoDefaultTimeout_BranchCoverage) {
  EXPECT_CALL(*mock, xStreamBufferCreate(64, 1))
      .WillOnce(Return(mock_stream_buffer_handle));
  EXPECT_CALL(*mock, xStreamBufferSend(mock_stream_buffer_handle, NotNull(), 4,
                                       portMAX_DELAY))
      .WillOnce(Return(4));
  EXPECT_CALL(*mock, vStreamBufferDelete(mock_stream_buffer_handle)).Times(1);
...
TEST_F(FreeRTOSStreamBufferTest,
       StreamBufferReceiveChronoDefaultTimeout_BranchCoverage) {
  EXPECT_CALL(*mock, xStreamBufferCreate(64, 1))
      .WillOnce(Return(mock_stream_buffer_handle));
  EXPECT_CALL(*mock, xStreamBufferReceive(mock_stream_buffer_handle, NotNull(),
                                          4, portMAX_DELAY))
...
TEST_F(FreeRTOSStreamBufferTest,
       StreamBufferSendIteratorsChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xStreamBufferCreate(256, 1))
      .WillOnce(Return(mock_stream_buffer_handle));
  EXPECT_CALL(*mock, xStreamBufferSend(mock_stream_buffer_handle, NotNull(), 3,
                                       pdMS_TO_TICKS(50)))
      .WillOnce(Return(3));
  EXPECT_CALL(*mock, vStreamBufferDelete(mock_stream_buffer_handle)).Times(1);
  ...
  auto result =
      buffer.send(data.begin(), data.end(), std::chrono::milliseconds(50));
  EXPECT_EQ(result, 3);
}

TEST_F(FreeRTOSStreamBufferTest, StreamBufferSendIsrNoWoken_BranchCoverage) {
  EXPECT_CALL(*mock, xStreamBufferCreate(64, 1))
      .WillOnce(Return(mock_stream_buffer_handle));
...
TEST_F(FreeRTOSStreamBufferTest,
       StreamBufferSendIsrIteratorsNoArg_BranchCoverage) {
  EXPECT_CALL(*mock, xStreamBufferCreate(64, 1))
      .WillOnce(Return(mock_stream_buffer_handle));
  EXPECT_CALL(*mock, xStreamBufferSendFromISR(mock_stream_buffer_handle,
                                              NotNull(), 3, _))
...
TEST_F(FreeRTOSStreamBufferTest, StreamBufferSendChrono_BranchCoverage) {
  ...
  freertos::da::stream_buffer<64> buffer;
  auto result = buffer.send(data, 4, std::chrono::milliseconds(100));
  EXPECT_EQ(result, 4);
}

TEST_F(FreeRTOSStreamBufferTest, StreamBufferReceiveChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xStreamBufferCreate(64, 1))
      .WillOnce(Return(mock_stream_buffer_handle));
  EXPECT_CALL(*mock, xStreamBufferReceive(mock_stream_buffer_handle, NotNull(),
                                          4, pdMS_TO_TICKS(200)))
