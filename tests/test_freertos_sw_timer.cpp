 ...
 *
 * This file provides comprehensive behavioral unit tests for the software timer
 * modules, including static and dynamic timer allocators.
 *
 * Tests cover:
 * - Correct construction, allocation, and deallocation of timers using static
 * and dynamic allocators
 * - Typical usage patterns for timers: creation, starting, stopping, and
 * deletion
 ...
 *
 * NOTE: Timer callbacks are normally executed by the FreeRTOS timer service
 * task. In host-based testing, callbacks are manually triggered in tests to
 * verify functionality. Some RTOS-specific timing behavior cannot be fully
 * replicated in host environment.
 *
 * RTOS-ONLY OR UNTESTABLE FEATURES:
 * =================================
 * 1. Actual timer callback execution by the timer service task
 *    - In real FreeRTOS, callbacks are invoked asynchronously by the timer
 * service task
 *    - Host testing can only verify callback registration, not automatic
 * invocation timing
 *
 ...
 *
 * These aspects require integration testing in the actual target RTOS
 * environment or hardware-in-the-loop testing for complete validation.
 */
...
protected:
  ...
  void triggerTimerCallback(TimerHandle_t timer_handle) {
    ...
    // In the actual implementation, we'd need to store and retrieve the
    // callback For now, we'll simulate by directly calling our test callback
    if (timer_id) {
      auto *timer_obj =
          static_cast<timer<static_sw_timer_allocator> *>(timer_id);
...
TEST_F(FreeRTOSSwTimerTest, StaticTimerAllocatorCreate) {
  ...
  EXPECT_CALL(
      *mock, xTimerCreateStatic(StrEq("TestTimer"), // timer name
                                100,                // period in ticks
                                pdTRUE,             // auto reload
...
TEST_F(FreeRTOSSwTimerTest, StaticTimerAllocatorCreateNullReturn) {
  ...
  TimerHandle_t handle =
      allocator.create("TestTimer", 100, pdTRUE, nullptr, test_timer_callback);
...
TEST_F(FreeRTOSSwTimerTest, DynamicTimerAllocatorCreate) {
  ...
  EXPECT_CALL(*mock,
              xTimerCreate(StrEq("TestTimer"), // timer name
                           200,                // period in ticks
                           pdFALSE,            // single shot
                           NotNull(), // timer ID (pointer to timer object)
                           NotNull()  // callback function
...
TEST_F(FreeRTOSSwTimerTest, DynamicTimerAllocatorCreateNullReturn) {
  ...
  TimerHandle_t handle =
      allocator.create("TestTimer", 200, pdFALSE, nullptr, test_timer_callback);
...
TEST_F(FreeRTOSSwTimerTest, StaticTimerConstructionWithChrono) {
  EXPECT_CALL(*mock,
              xTimerCreateStatic(_, 500, _, _, _, _)) // 500ms = 500 ticks
      .WillOnce(Return(mock_timer_handle));
...
TEST_F(FreeRTOSSwTimerTest, StaticTimerStartFromISR) {
  ...
  auto result = test_timer.start_isr();
...
TEST_F(FreeRTOSSwTimerTest, StaticTimerStopFromISR) {
  ...
  auto result = test_timer.stop_isr();
  EXPECT_EQ(result.result, pdPASS);
}
...
TEST_F(FreeRTOSSwTimerTest, StaticTimerResetFromISR) {
  ...
  auto result = test_timer.reset_isr();
  EXPECT_EQ(result.result, pdPASS);
}
...
TEST_F(FreeRTOSSwTimerTest, StaticTimerPeriodFromISR) {
  ...
  expectDestructor();
  EXPECT_CALL(*mock,
              xTimerChangePeriodFromISR(mock_timer_handle, 2000, NotNull()))
      .WillOnce(Return(pdPASS));

  auto result = test_timer.period_isr(2000);
  EXPECT_EQ(result.result, pdPASS);
}
...
TEST_F(FreeRTOSSwTimerTest, StaticTimerRemainingTime) {
  ...
  EXPECT_CALL(*mock, xTaskGetTickCount()).Times(2).WillRepeatedly(Return(3000));
...
TEST_F(FreeRTOSSwTimerTest, TimerOperationsWithNullHandle) {
  ...
  EXPECT_EQ(test_timer.start_isr().result, pdFAIL);
  EXPECT_EQ(test_timer.stop_isr().result, pdFAIL);
  EXPECT_EQ(test_timer.reset_isr().result, pdFAIL);
  EXPECT_EQ(test_timer.period_isr(2000).result, pdFAIL);
...
TEST_F(FreeRTOSSwTimerTest, TimerFailureConditions) {
  ...
  EXPECT_CALL(*mock, xTimerStop(mock_timer_handle, _)).WillOnce(Return(pdFAIL));
  EXPECT_EQ(test_timer.stop(), pdFAIL);
...
TEST_F(FreeRTOSSwTimerTest, TimerMoveConstructionIssueScenario) {
...

  {
    ...
    TimerContainer container(
        sa::timer("TestTimer", 1000, pdFALSE, std::move(callback)));
...
TEST_F(FreeRTOSSwTimerTest, TimerMoveConstruction) {
  ...
  // With proper move constructor, only the moved-to object should delete the
  // timer
  EXPECT_CALL(*mock, xTimerDelete(mock_timer_handle, portMAX_DELAY))
...
TEST_F(FreeRTOSSwTimerTest, TimerMoveAssignment) {
  ...
  // Source timer properties should be queried before deletion
  EXPECT_CALL(*mock, pcTimerGetName(mock_timer_handle))
      .WillOnce(Return("SourceTimer"));
  EXPECT_CALL(*mock, xTimerGetPeriod(mock_timer_handle)).WillOnce(Return(1000));
  EXPECT_CALL(*mock, uxTimerGetReloadMode(mock_timer_handle))
      .WillOnce(Return(pdTRUE));
...
TEST_F(FreeRTOSSwTimerTest, ChronoCompatibility) {
  ...
  EXPECT_CALL(*mock,
              xTimerCreateStatic(_, 1000, _, _, _, _)) // 1 second = 1000ms
      .WillOnce(Return(mock_timer_handle));
  ...
  EXPECT_CALL(*mock,
              xTimerChangePeriodFromISR(mock_timer_handle, 1500, NotNull()))
      .WillOnce(Return(pdPASS));
  auto period_isr_result = test_timer.period_isr(1500ms);
  EXPECT_EQ(period_isr_result.result, pdPASS);

  EXPECT_CALL(*mock,
              xTimerChangePeriodFromISR(mock_timer_handle, 800, NotNull()))
      .WillOnce(Return(pdPASS));
  auto period_isr_result2 = test_timer.period_isr(800ms);
  EXPECT_EQ(period_isr_result2.result, pdPASS);
}
...
TEST_F(FreeRTOSSwTimerTest, PeriodChangeWithVariousDurationCombinations) {
  ...
  EXPECT_CALL(*mock, xTimerChangePeriod(mock_timer_handle, 5000, 2000))
      .WillOnce(Return(pdPASS));
  BaseType_t result3 = test_timer.period(std::chrono::seconds(5),
...
TEST_F(FreeRTOSSwTimerTest, PeriodChangeWithMicrosecondPrecision) {
  ...
  EXPECT_CALL(*mock, xTimerChangePeriod(mock_timer_handle, 1500, 500))
      .WillOnce(Return(pdPASS));
  BaseType_t result1 =
      test_timer.period(std::chrono::microseconds(1500000), 500ms);
  EXPECT_EQ(result1, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, PeriodChangeWithMinutePrecision) {
  ...
  EXPECT_CALL(*mock, xTimerChangePeriod(mock_timer_handle, 10000, 30000))
      .WillOnce(Return(pdPASS));
  BaseType_t result2 = test_timer.period(
      10s, std::chrono::duration<double, std::ratio<60>>(0.5));
  EXPECT_EQ(result2, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, PeriodChangeWithFloatingPointDurations) {
  ...
  EXPECT_CALL(*mock, xTimerChangePeriod(mock_timer_handle, 2500, 1500))
      .WillOnce(Return(pdPASS));
  BaseType_t result1 =
      test_timer.period(std::chrono::duration<double>(2.5), 1500ms);
  EXPECT_EQ(result1, pdPASS);
  ...
  EXPECT_CALL(*mock, xTimerChangePeriod(mock_timer_handle, 1234, 3000))
      .WillOnce(Return(pdPASS));
  BaseType_t result2 = test_timer.period(
      std::chrono::duration<double, std::milli>(1234.567), 3s);
  EXPECT_EQ(result2, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, PeriodChangeEdgeCaseDurations) {
  ...
  EXPECT_CALL(*mock, xTimerChangePeriod(mock_timer_handle, 0, 1))
      .WillOnce(Return(pdPASS));
  BaseType_t result2 = test_timer.period(std::chrono::microseconds(500),
                                         1ms); // 0.5ms rounds to 0
  EXPECT_EQ(result2, pdPASS);

  // Test maximum practical durations
  // Use large but reasonable values to avoid overflow
  EXPECT_CALL(*mock, xTimerChangePeriod(mock_timer_handle, 3600000,
                                        1800000)) // 1 hour, 30 minutes
      .WillOnce(Return(pdPASS));
  BaseType_t result3 =
      test_timer.period(std::chrono::hours(1), std::chrono::minutes(30));
  EXPECT_EQ(result3, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, ConstructorWithVariousDurationTypes) {
  ...
  EXPECT_CALL(*mock,
              xTimerCreateStatic(_, 2500, _, _, _, _)) // 2.5 seconds = 2500ms
      .WillOnce(Return(handle1));

  auto callback1 = createTestCallback();
  sa::timer test_timer1("TestTimer1", std::chrono::duration<double>(2.5),
                        pdTRUE, std::move(callback1));
  expectDestructor(handle1);

  // Test constructor with minutes
  TimerHandle_t handle2 = reinterpret_cast<TimerHandle_t>(0x22222222);
  EXPECT_CALL(*mock,
              xTimerCreateStatic(_, 120000, _, _, _, _)) // 2 minutes = 120000ms
      .WillOnce(Return(handle2));

  auto callback2 = createTestCallback();
  sa::timer test_timer2("TestTimer2", std::chrono::minutes(2), pdFALSE,
                        std::move(callback2));
  ...
  EXPECT_CALL(*mock, xTimerCreateStatic(_, 5000, _, _, _,
                                        _)) // 5000000 microseconds = 5000ms
      .WillOnce(Return(handle3));

  auto callback3 = createTestCallback();
  sa::timer test_timer3("TestTimer3", std::chrono::microseconds(5000000),
                        pdTRUE, std::move(callback3));
...
TEST_F(FreeRTOSSwTimerTest, StartStopResetWithMixedDurationTypes) {
  ...
  EXPECT_CALL(*mock,
              xTimerStop(mock_timer_handle, 750)) // 750000 microseconds = 750ms
      .WillOnce(Return(pdPASS));
  ...
  EXPECT_CALL(*mock,
              xTimerReset(mock_timer_handle, 1500)) // 1.5 seconds = 1500ms
      .WillOnce(Return(pdPASS));
...
TEST_F(FreeRTOSSwTimerTest, ISRFunctionsWithMixedDurationTypes) {
  ...
  EXPECT_CALL(*mock, xTimerChangePeriodFromISR(mock_timer_handle, 5000,
                                               NotNull())) // 5 seconds = 5000ms
      .WillOnce(Return(pdPASS));
  auto result1 = test_timer.period_isr(5s);
  EXPECT_EQ(result1.result, pdPASS);

  // Test period_isr with microseconds (no high_priority_woken parameter)
  EXPECT_CALL(*mock, xTimerChangePeriodFromISR(
                         mock_timer_handle, 3000,
                         NotNull())) // 3000000 microseconds = 3000ms
      .WillOnce(Return(pdPASS));
  auto result2 = test_timer.period_isr(std::chrono::microseconds(3000000));
  EXPECT_EQ(result2.result, pdPASS);

  // Test period_isr with floating point duration
  EXPECT_CALL(*mock,
              xTimerChangePeriodFromISR(mock_timer_handle, 7500,
                                        NotNull())) // 7.5 seconds = 7500ms
      .WillOnce(Return(pdPASS));
  auto result3 = test_timer.period_isr(std::chrono::duration<double>(7.5));
  EXPECT_EQ(result3.result, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, DestructorWithActiveTimer) {
  ...
  // Destructor just calls xTimerDelete and nulls the handle,
  // no busy-wait loop anymore
  EXPECT_CALL(*mock, xTimerDelete(mock_timer_handle, portMAX_DELAY))
      .WillOnce(Return(pdPASS));
...
// Note: The following features are RTOS-specific and cannot be fully tested in
// host environment:
// 1. Actual timer callback execution by the timer service task
// 2. Real-time timer expiry and automatic callback invocation
...
TEST_F(FreeRTOSSwTimerTest, StartIsrNoWoken_BranchCoverage) {
  ...
  sa::timer t("test", 100, pdFALSE, nullptr);
  EXPECT_EQ(t.start_isr().result, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, StopIsrNoWoken_BranchCoverage) {
  ...
  sa::timer t("test", 100, pdFALSE, nullptr);
  t.start(0);
  EXPECT_EQ(t.stop_isr().result, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, ResetIsrNoWoken_BranchCoverage) {
  ...
  sa::timer t("test", 100, pdFALSE, nullptr);
  t.start(0);
  EXPECT_EQ(t.reset_isr().result, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, PeriodIsrNoWoken_BranchCoverage) {
  ...
  sa::timer t("test", 100, pdFALSE, nullptr);
  t.start(0);
  EXPECT_EQ(t.period_isr(200).result, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, PeriodIsrChronoNoWoken_BranchCoverage) {
  ...
  sa::timer t("test", 100, pdFALSE, nullptr);
  t.start(0);
  EXPECT_EQ(t.period_isr(std::chrono::milliseconds(200)).result, pdPASS);
...
TEST_F(FreeRTOSSwTimerTest, PeriodIsrChronoWithWoken_BranchCoverage) {
  ...
  sa::timer t("test", 100, pdFALSE, nullptr);
  EXPECT_EQ(t.period_isr(std::chrono::milliseconds(1500)).result, pdPASS);
}
...
// =============================================================================
// DYNAMIC TIMER MOVE AND SWAP TESTS
// =============================================================================

#if configSUPPORT_DYNAMIC_ALLOCATION

TEST_F(FreeRTOSSwTimerTest, DynamicTimerMoveConstruction) {
  TimerHandle_t handle1 = reinterpret_cast<TimerHandle_t>(0x1111);

  EXPECT_CALL(*mock, xTimerCreate(_, _, _, _, _)).WillOnce(Return(handle1));
  EXPECT_CALL(*mock, vTimerSetTimerID(handle1, _));
  EXPECT_CALL(*mock, xTimerDelete(handle1, portMAX_DELAY))
      .WillOnce(Return(pdPASS));

  auto callback = createTestCallback();
  freertos::timer<freertos::dynamic_sw_timer_allocator> t1(
      "Timer1", 1000, pdTRUE, std::move(callback));
  freertos::timer<freertos::dynamic_sw_timer_allocator> t2(std::move(t1));
}

TEST_F(FreeRTOSSwTimerTest, DynamicTimerMoveAssignment) {
  TimerHandle_t handle1 = reinterpret_cast<TimerHandle_t>(0x1111);
  ...
  auto callback2 = createTestCallback();
  freertos::timer<freertos::dynamic_sw_timer_allocator> t1(
      "Timer1", 1000, pdTRUE, std::move(callback1));
  freertos::timer<freertos::dynamic_sw_timer_allocator> t2(
      "Timer2", 2000, pdFALSE, std::move(callback2));

  EXPECT_CALL(*mock, xTimerDelete(handle2, portMAX_DELAY))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, xTimerStop(handle1, portMAX_DELAY))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcTimerGetName(handle1)).WillOnce(Return("Timer1"));
  EXPECT_CALL(*mock, xTimerGetPeriod(handle1)).WillOnce(Return(1000));
  EXPECT_CALL(*mock, uxTimerGetReloadMode(handle1)).WillOnce(Return(pdTRUE));
  EXPECT_CALL(*mock, xTimerDelete(handle1, portMAX_DELAY))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, xTimerCreate(_, _, _, _, _)).WillOnce(Return(new_handle));
  EXPECT_CALL(*mock, xTimerDelete(new_handle, portMAX_DELAY))
      .WillOnce(Return(pdPASS));
...
TEST_F(FreeRTOSSwTimerTest, DynamicTimerSwap) {
  TimerHandle_t handle1 = reinterpret_cast<TimerHandle_t>(0x1111);
  TimerHandle_t handle2 = reinterpret_cast<TimerHandle_t>(0x2222);
