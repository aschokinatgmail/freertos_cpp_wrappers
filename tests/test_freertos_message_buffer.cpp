 ...
 *
 * Tests cover:
 * - Correct construction, allocation, and deallocation using static and dynamic
 * allocators
 ...
 * - Error and edge cases, including buffer full/empty conditions, timeouts,
 * invalid params
 ...
 * - Actual timeout behavior depends on FreeRTOS scheduler (host testing uses
 * mock return values)
 * - ISR variants of functions are not testable in host environment but are
 * mocked for completeness
 * - Real message ordering and thread safety depend on FreeRTOS kernel (host
 * testing simulates)
 * - Actual memory layout and buffer overflow protection are RTOS-specific
 * features
 */
...
protected:
  void SetUp() override {
    ...
    mock_message_buffer_handle =
        reinterpret_cast<MessageBufferHandle_t>(0x12345678);
  }
...
TEST_F(FreeRTOSMessageBufferTest, StaticMessageBufferAllocatorConstruction) {
  ...
  // static_message_buffer_allocator<512> copy_allocator(allocator);  // Should
  // not compile
...
TEST_F(FreeRTOSMessageBufferTest, StaticMessageBufferAllocatorCreate) {
  ...
  EXPECT_CALL(*mock,
              xMessageBufferCreateStatic(512,       // buffer size
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferSendSuccess) {
  ...
  EXPECT_CALL(*mock, xMessageBufferSend(mock_message_buffer_handle, test_data,
                                        data_length,
                                        100 // timeout ticks
                                        ))
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferSendWithChrono) {
  ...
  // Test chrono duration conversion: 100ms should convert to 100 ticks (with
  // portTICK_PERIOD_MS = 1)
  EXPECT_CALL(*mock, xMessageBufferSend(mock_message_buffer_handle, &test_value,
                                        sizeof(test_value),
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferSendZeroLengthMessage) {
  ...
  EXPECT_CALL(*mock,
              xMessageBufferSend(mock_message_buffer_handle, nullptr, 0, 10))
      .WillOnce(Return(0));
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferSendMaxSizeMessage) {
  ...
  EXPECT_CALL(*mock,
              xMessageBufferSend(mock_message_buffer_handle, large_data.data(),
                                 large_data.size(), portMAX_DELAY))
      .WillOnce(Return(large_data.size()));

  size_t bytes_sent =
      test_buffer.send(large_data.data(), large_data.size(), portMAX_DELAY);
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferReceiveSuccess) {
  ...
  EXPECT_CALL(*mock,
              xMessageBufferReceive(mock_message_buffer_handle, receive_buffer,
  ...
  size_t bytes_received =
      test_buffer.receive(receive_buffer, sizeof(receive_buffer), 100);
  EXPECT_EQ(bytes_received, expected_length);

  // For message buffers, we need to null-terminate manually if treating as
  // string
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferReceiveTimeout) {
  ...
  size_t bytes_received =
      test_buffer.receive(receive_buffer, sizeof(receive_buffer), 50);
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferReceiveWithChrono) {
  ...
  EXPECT_CALL(*mock,
              xMessageBufferReceive(mock_message_buffer_handle, &receive_value,
  ...
  size_t bytes_received =
      test_buffer.receive(&receive_value, sizeof(receive_value), 250ms);
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferReceiveBufferTooSmall) {
  ...
  size_t bytes_received =
      test_buffer.receive(small_buffer, sizeof(small_buffer), 100);
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferCreationFailure) {
  ...
  // Note: The actual wrapper may assert or handle this differently in a real
  // RTOS environment
}
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferChronoSecondsTimeout) {
  ...
  size_t bytes_received =
      test_buffer.receive(receive_buffer, sizeof(receive_buffer), 2s);
  EXPECT_EQ(bytes_received, 0);
...
TEST_F(FreeRTOSMessageBufferTest, StaticVsDynamicBehaviorComparison) {
  ...
  // Static buffer test
  {
    ...
    EXPECT_CALL(*mock,
                xMessageBufferSend(mock_message_buffer_handle, test_message,
                                   strlen(test_message), 100))
        .WillOnce(Return(strlen(test_message)));

    size_t static_bytes_sent =
        static_buffer.send(test_message, strlen(test_message), 100);
  ...
  {
    MessageBufferHandle_t dynamic_handle =
    ...
    EXPECT_CALL(*mock, xMessageBufferSend(dynamic_handle, test_message,
                                          strlen(test_message), 100))
        .WillOnce(Return(strlen(test_message)));

    size_t dynamic_bytes_sent =
        dynamic_buffer.send(test_message, strlen(test_message), 100);
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferComplexSendReceiveScenario) {
  ...
  EXPECT_CALL(*mock, xMessageBufferSend(mock_message_buffer_handle, msg1,
                                        strlen(msg1), 50))
  ...
  EXPECT_CALL(*mock, xMessageBufferSend(mock_message_buffer_handle, msg2_data,
                                        sizeof(msg2_data), 50))
  ...
  size_t received2 =
      test_buffer.receive(recv_buffer2, sizeof(recv_buffer2), 50);
...
TEST_F(FreeRTOSMessageBufferTest, MessageBufferSendChronoOnly_BranchCoverage) {
  EXPECT_CALL(*mock, xMessageBufferCreateStatic(_, _, _))
      .WillOnce(Return(mock_message_buffer_handle));

  sa::message_buffer<256> test_buffer;

  const char data[] = "chrono";
  EXPECT_CALL(*mock, xMessageBufferSend(mock_message_buffer_handle, data,
                                        strlen(data), pdMS_TO_TICKS(150)))
      .WillOnce(Return(strlen(data)));

  size_t bytes_sent =
      test_buffer.send(data, strlen(data), std::chrono::milliseconds(150));
...
TEST_F(FreeRTOSMessageBufferTest,
       MessageBufferReceiveChronoOnly_BranchCoverage) {
  EXPECT_CALL(*mock, xMessageBufferCreateStatic(_, _, _))
      .WillOnce(Return(mock_message_buffer_handle));
  ...
  char buf[64];
  EXPECT_CALL(*mock, xMessageBufferReceive(mock_message_buffer_handle, buf,
                                           sizeof(buf), pdMS_TO_TICKS(200)))
      .WillOnce(Return(5));

  size_t bytes_received =
      test_buffer.receive(buf, sizeof(buf), std::chrono::milliseconds(200));
  EXPECT_EQ(bytes_received, 5);

  EXPECT_CALL(*mock, vMessageBufferDelete(mock_message_buffer_handle));
}

TEST_F(FreeRTOSMessageBufferTest, MessageBufferSendChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xMessageBufferCreateStatic(_, _, _))
      .WillOnce(Return(mock_message_buffer_handle));
  ...
  EXPECT_CALL(*mock, xMessageBufferSend(mock_message_buffer_handle, NotNull(),
                                        strlen(data), pdMS_TO_TICKS(100)))
      .WillOnce(Return(strlen(data)));

  size_t bytes_sent =
      test_buffer.send(data, strlen(data), std::chrono::milliseconds(100));
  EXPECT_EQ(bytes_sent, strlen(data));
...
TEST_F(FreeRTOSMessageBufferTest,
       MessageBufferStaticSendChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xMessageBufferCreateStatic(_, _, _))
      .WillOnce(Return(mock_message_buffer_handle));
  ...
  EXPECT_CALL(*mock, xMessageBufferSend(mock_message_buffer_handle, NotNull(),
                                        strlen(data), pdMS_TO_TICKS(250)))
      .WillOnce(Return(strlen(data)));

  size_t bytes_sent =
      test_buffer.send(data, strlen(data), std::chrono::milliseconds(250));
  EXPECT_EQ(bytes_sent, strlen(data));
