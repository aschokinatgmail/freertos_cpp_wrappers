...
protected:
  void SetUp() override {
    ...
    ON_CALL(*mock, xEventGroupCreateStatic(_))
        .WillByDefault(
            Return(reinterpret_cast<EventGroupHandle_t>(0x22222222)));
  }
...
TEST_F(Cpp17FeaturesTest, TaskMoveSemanticsPerfectForwarding) {
  ...
  EXPECT_CALL(*mock, xTaskCreateStatic(_, _, _, _, _, _, _)).Times(AtLeast(2));
...
TEST_F(Cpp17FeaturesTest, TaskMoveAssignmentChaining) {
  ...
  EXPECT_CALL(*mock, xTaskCreateStatic(_, _, _, _, _, _, _)).Times(AtLeast(3));
...
TEST_F(Cpp17FeaturesTest, PeriodicTaskMoveWithChronoTypes) {
  ...
  sa::periodic_task<1024> task1("PeriodicMove1", 1, std::move(on_start),
                                std::move(on_stop), std::move(periodic));

  // Create another task and move it
  sa::periodic_task<1024> task2(
      "PeriodicMove2", 2, [&start_count]() { start_count++; },
...
TEST_F(Cpp17FeaturesTest, LambdaCaptureVarieties) {
  ...
  sa::task<1024> task1("RefCapture", 1,
                       [&message]() { EXPECT_EQ(message, "Test message"); });

  // Capture by value
  sa::task<1024> task2("ValueCapture", 1,
                       [counter]() { EXPECT_EQ(counter, 42); });
...
TEST_F(Cpp17FeaturesTest, RAIIExceptionSafety) {
  ...
  EXPECT_CALL(*mock, vTaskDelete(_)).Times(AtLeast(1));
...
TEST_F(Cpp17FeaturesTest, TypeTraitsValidation) {
  ...
  EXPECT_TRUE(std::is_move_assignable_v<sa::task<1024>>);
  ...
  EXPECT_TRUE(std::is_move_assignable_v<sa::periodic_task<1024>>);
...
TEST_F(Cpp17FeaturesTest, TemplateDeductionGuides) {
  EXPECT_CALL(*mock, xQueueCreateStatic(_, _, _, _)).Times(AtLeast(2));
...
TEST_F(Cpp17FeaturesTest, CompileTimeConstants) {
  ...
  EXPECT_CALL(*mock, xTaskCreateStatic(_, _, _, _, priority, _, _)).Times(1);
  EXPECT_CALL(*mock, xQueueCreateStatic(queue_size, _, _, _)).Times(1);

  // These should compile with constexpr values - queues are created at
  // construction
...
TEST_F(Cpp17FeaturesTest, ChronoTypesIntegration) {
  ...
  // Test with different chrono duration types in periodic tasks - using static
  // allocation
  sa::periodic_task<1024> task1(
      "ChronoMs", 1, []() { /* start */ }, []() { /* stop */ },
      []() { /* periodic */ }, 500ms);

  sa::periodic_task<1024> task2(
      "ChronoMs2", 1, []() { /* start */ }, []() { /* stop */ },
      []() { /* periodic */ }, 750ms);

  sa::periodic_task<1024> task3(
      "ChronoCustom", 1, []() { /* start */ }, []() { /* stop */ },
      []() { /* periodic */ }, duration<double, std::milli>(123.456));
...
TEST_F(Cpp17FeaturesTest, UniquePtrIntegration) {
  EXPECT_CALL(*mock, xTaskCreateStatic(_, _, _, _, _, _, _)).Times(AtLeast(2));
  EXPECT_CALL(*mock, vTaskDelete(_)).Times(AtLeast(2));

  // Test with unique_ptr for dynamic task management
  auto task1 =
      std::make_unique<sa::task<1024>>("UniqueTask1", 1, []() { /* test */ });
  auto task2 =
      std::make_unique<sa::task<1024>>("UniqueTask2", 2, []() { /* test */ });
