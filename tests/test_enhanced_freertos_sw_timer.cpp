 ...
 * @brief Enhanced unit tests for FreeRTOS C++ software timer wrappers with
 * realistic timer service simulation
 *
 * This file provides enhanced tests that demonstrate realistic FreeRTOS timer
 * behavior using the enhanced timer service simulator. Unlike the basic timer
 * tests, these tests actually simulate timer service task behavior including:
 *
 ...
 *
 * These tests provide comprehensive coverage of timer behavior that closely
 * mirrors what would happen in a real FreeRTOS environment with an actual timer
 * service task.
 */
...
TEST_F(EnhancedFreeRTOSSwTimerTest, ZeroPeriodTimer) {
  ...
  TimerHandle_t handle =
      sim.createTimer("ZeroPeriod", 0, pdTRUE, nullptr, [](TimerHandle_t) {});
...
TEST_F(EnhancedFreeRTOSSwTimerTest, NullCallbackTimer) {
  ...
  auto &sim = enhanced_mock->getSimulator();
  TimerHandle_t handle =
      sim.createTimer("NullCallback", 1000, pdTRUE, nullptr, nullptr);
  EXPECT_EQ(handle, nullptr);
}
...
TEST_F(EnhancedFreeRTOSSwTimerTest, ISRFunctionsEnhancedBehavior) {
  ...
  // Test ISR start
  auto isr_res1 = test_timer.start_isr();
  EXPECT_EQ(isr_res1.result, pdPASS);
  EXPECT_TRUE(test_timer.running());
  ...
  // Test ISR stop
  auto isr_res2 = test_timer.stop_isr();
  EXPECT_EQ(isr_res2.result, pdPASS);
  EXPECT_FALSE(test_timer.running());
  ...
  // Test ISR reset
  auto isr_res3 = test_timer.reset_isr();
  EXPECT_EQ(isr_res3.result, pdPASS);
  EXPECT_TRUE(test_timer.running());
...
TEST_F(EnhancedFreeRTOSSwTimerTest, ComprehensiveIntegrationTest) {
  ...
  sa::timer heartbeat_timer("Heartbeat", 50, pdTRUE,
                            std::move(heartbeat_callback));
  ...
  sa::timer cleanup_timer("Cleanup", 1000, pdFALSE,
...
TEST_F(EnhancedFreeRTOSSwTimerTest, ManyTimersStressTest) {
  ...
  for (int i = 0; i < NUM_TIMERS; ++i) {
    ...
    timers.push_back(
        std::make_unique<sa::timer>(("Timer" + std::to_string(i)).c_str(),
                                    period, pdTRUE, std::move(callback)));
