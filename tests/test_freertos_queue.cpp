 ...
 * wrapper library, including static and dynamic queue allocators with
 * comprehensive queue operations testing.
 *
 * Test Coverage:
 * - Static and dynamic queue allocators
 * - Queue creation and destruction (including registry management)
 * - Send operations (send, send_back, send_front) with timeouts and ISR
 * variants
 * - Receive operations with optional<T> and reference variants
 * - Peek operations (non-destructive read)
 * - Queue state operations (messages_waiting, spaces_available, empty/full
 * checks)
...
TEST_F(FreeRTOSQueueTest, StaticQueueAllocatorCreate) {
  ...
  EXPECT_CALL(*mock,
              xQueueCreateStatic(5, sizeof(uint32_t), NotNull(), NotNull()))
...
TEST_F(FreeRTOSQueueTest, QueueConstructionWithoutRegistry) {
  ...
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle)).Times(1);
...
TEST_F(FreeRTOSQueueTest, QueueConstructionWithRegistry) {
  ...
  EXPECT_CALL(*mock, vQueueUnregisterQueue(mock_queue_handle)).Times(1);
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle)).Times(1);

  freertos::queue<8, uint32_t, freertos::dynamic_queue_allocator<8, uint32_t>>
      queue(queue_name);
...
TEST_F(FreeRTOSQueueTest, QueueDestructionNullHandle) {
   ...
   */
  EXPECT_CALL(*mock, xQueueCreate(3, sizeof(double))).WillOnce(Return(nullptr));
  ...
  try {
    freertos::queue<3, double, freertos::dynamic_queue_allocator<3, double>>
        queue;
...
TEST_F(FreeRTOSQueueTest, QueueSendBasic) {
  ...
  freertos::queue<4, uint16_t, freertos::dynamic_queue_allocator<4, uint16_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueSendBack) {
  EXPECT_CALL(*mock, xQueueCreate(6, sizeof(double)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock,
              xQueueSendToBack(mock_queue_handle, NotNull(), portMAX_DELAY))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));

  freertos::queue<6, double, freertos::dynamic_queue_allocator<6, double>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueSendTimeout) {
  ...
  freertos::queue<2, uint8_t, freertos::dynamic_queue_allocator<2, uint8_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueSendChronoTimeout) {
  EXPECT_CALL(*mock, xQueueCreate(4, sizeof(uint16_t)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueSend(mock_queue_handle, NotNull(),
                                500)) // 500ms converted to ticks
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, QueueReceiveOptional) {
  ...
  freertos::queue<3, uint16_t, freertos::dynamic_queue_allocator<3, uint16_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueReceiveISR) {
  EXPECT_CALL(*mock, xQueueCreate(6, sizeof(float)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock,
              xQueueReceiveFromISR(mock_queue_handle, NotNull(), NotNull()))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, QueueReceiveChronoTimeout) {
  EXPECT_CALL(*mock, xQueueCreate(4, sizeof(uint32_t)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueReceive(mock_queue_handle, NotNull(),
  ...
  freertos::queue<4, uint32_t, freertos::dynamic_queue_allocator<4, uint32_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueuePeekISR) {
  ...
  freertos::queue<4, double, freertos::dynamic_queue_allocator<4, double>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueSpacesAvailable) {
  ...
  freertos::queue<10, uint32_t, freertos::dynamic_queue_allocator<10, uint32_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueIsEmpty) {
  ...
  freertos::queue<4, uint8_t, freertos::dynamic_queue_allocator<4, uint8_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueIsFull) {
  ...
  freertos::queue<2, uint32_t, freertos::dynamic_queue_allocator<2, uint32_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueGetName) {
  ...
  EXPECT_CALL(*mock, xQueueCreate(5, sizeof(uint32_t)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock,
              vQueueAddToRegistry(mock_queue_handle, StrEq(expected_name)))
      .Times(1);
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
...
TEST_F(FreeRTOSQueueTest, QueueReset) {
  EXPECT_CALL(*mock, xQueueCreate(6, sizeof(float)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueReset(mock_queue_handle)).WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, QueueOverwrite) {
  EXPECT_CALL(
      *mock,
      xQueueCreate(1, sizeof(uint32_t))) // Single item queue for overwrite
      .WillOnce(Return(mock_queue_handle));
  ...
  freertos::queue<1, uint32_t, freertos::dynamic_queue_allocator<1, uint32_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueOverwriteISR) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(uint16_t)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock,
              xQueueOverwriteFromISR(mock_queue_handle, NotNull(), NotNull()))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));

  freertos::queue<1, uint16_t, freertos::dynamic_queue_allocator<1, uint16_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, LargeQueueSize) {
  ...
  freertos::queue<large_size, uint64_t,
                  freertos::dynamic_queue_allocator<large_size, uint64_t>>
      large_queue;
...
TEST_F(FreeRTOSQueueTest, ZeroSizeQueue) {
  ...
  try {
    freertos::queue<0, uint32_t, freertos::dynamic_queue_allocator<0, uint32_t>>
        zero_queue;
...
TEST_F(FreeRTOSQueueTest, ComplexDataTypeQueue) {
  ...
  freertos::queue<3, ComplexTestData,
                  freertos::dynamic_queue_allocator<3, ComplexTestData>>
      complex_queue;
...
TEST_F(FreeRTOSQueueTest, QueueWithVariousDataTypes) {
  ...
  freertos::queue<5, int8_t, freertos::dynamic_queue_allocator<5, int8_t>>
      int8_queue;
  ...
  freertos::queue<3, int16_t, freertos::dynamic_queue_allocator<3, int16_t>>
      int16_queue;
  ...
  freertos::queue<2, int64_t, freertos::dynamic_queue_allocator<2, int64_t>>
      int64_queue;
}
...
TEST_F(FreeRTOSQueueTest, QueueWithEnumTypes) {
  ...
  freertos::queue<4, Priority, freertos::dynamic_queue_allocator<4, Priority>>
      priority_queue;
...
TEST_F(FreeRTOSQueueTest, QueueWithPointerTypes) {
  ...
  freertos::queue<10, DataPtr, freertos::dynamic_queue_allocator<10, DataPtr>>
      ptr_queue;
...
TEST_F(FreeRTOSQueueTest, QueueChronoMicrosecondsTimeout) {
  EXPECT_CALL(*mock, xQueueCreate(3, sizeof(float)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueReceive(mock_queue_handle, NotNull(),
                                   0)) // 500µs = 0ms after duration_cast
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, QueueChronoMinutesTimeout) {
  ...
  freertos::queue<2, double, freertos::dynamic_queue_allocator<2, double>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueChronoNanosecondsTimeout) {
  EXPECT_CALL(*mock, xQueueCreate(5, sizeof(uint64_t)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock,
              xQueuePeek(mock_queue_handle, NotNull(), 1)) // 1000000ns = 1ms
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));

  freertos::queue<5, uint64_t, freertos::dynamic_queue_allocator<5, uint64_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueChronoOptionalReceiveVariousDurations) {
  ...
  freertos::queue<4, int32_t, freertos::dynamic_queue_allocator<4, int32_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueISROperationsWithDifferentTypes) {
  ...
  EXPECT_CALL(*mock,
              xQueueReceiveFromISR(mock_queue_handle, NotNull(), NotNull()))
      .Times(2) // Called twice: once for receive_isr(ref) and once for
                // receive_isr()
  ...
  freertos::queue<6, uint16_t, freertos::dynamic_queue_allocator<6, uint16_t>>
      queue;
...
TEST_F(FreeRTOSQueueTest, QueueSendBackISROperations) {
  EXPECT_CALL(*mock, xQueueCreate(4, sizeof(uint32_t)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock,
              xQueueSendToBackFromISR(mock_queue_handle, NotNull(), NotNull()))
...
TEST_F(FreeRTOSQueueTest, QueueMessagesWaitingISR) {
  ...
  freertos::queue<5, double, freertos::dynamic_queue_allocator<5, double>>
      queue;

  /*
   * ISR Messages Waiting Test:
   * Tests the ISR-safe version of messages_waiting(). In a real FreeRTOS
   * environment, this function would be called from an interrupt service
   * routine where normal API functions cannot be used safely.
   */
...
// =============================================================================
// Branch Coverage Tests - Additional paths
// =============================================================================

TEST_F(FreeRTOSQueueTest, ReceiveIsrOptionalEmptyOnFail_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueReceiveFromISR(mock_queue_handle, _, _))
      .WillOnce(Return(pdFALSE));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, PeekTickTimeoutFails_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueuePeek(mock_queue_handle, _, portMAX_DELAY))
      .WillOnce(Return(pdFALSE));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto result = q.peek(portMAX_DELAY);
  EXPECT_FALSE(result.has_value());
}

TEST_F(FreeRTOSQueueTest, PeekChronoSuccess_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueuePeek(mock_queue_handle, _, pdMS_TO_TICKS(100)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, PeekChronoFails_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueuePeek(mock_queue_handle, _, pdMS_TO_TICKS(100)))
      .WillOnce(Return(pdFALSE));
...
TEST_F(FreeRTOSQueueTest, PeekIsrEmpty_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueuePeekFromISR(mock_queue_handle, _, _))
      .WillOnce(Return(pdFALSE));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, OverwriteIsrNoWoken_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val = 42;
  EXPECT_CALL(*mock, xQueueOverwriteFromISR(mock_queue_handle, _, _))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, SendBackChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val = 42;
  EXPECT_CALL(*mock, xQueueSendToBack(mock_queue_handle, _, pdMS_TO_TICKS(100)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, SendFrontChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val = 42;
  EXPECT_CALL(*mock,
              xQueueSendToFront(mock_queue_handle, _, pdMS_TO_TICKS(100)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, ReceiveChronoWithValue_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueReceive(mock_queue_handle, _, pdMS_TO_TICKS(100)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, FullIsrReturnsFalse_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueIsQueueFullFromISR(mock_queue_handle))
      .WillOnce(Return(pdFALSE));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  EXPECT_FALSE(q.full_isr());
}

TEST_F(FreeRTOSQueueTest, EmptyIsrReturnsFalse_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueIsQueueEmptyFromISR(mock_queue_handle))
      .WillOnce(Return(pdFALSE));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  EXPECT_FALSE(q.empty_isr());
}

TEST_F(FreeRTOSQueueTest, PeekIsrWithRefFails_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueuePeekFromISR(mock_queue_handle, _, _))
      .WillOnce(Return(pdFALSE));
...
TEST_F(FreeRTOSQueueTest, SendChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val = 42;
  EXPECT_CALL(*mock, xQueueSend(mock_queue_handle, _, pdMS_TO_TICKS(250)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto rc = q.send(val, std::chrono::milliseconds(250));
  EXPECT_EQ(rc, pdPASS);
}

TEST_F(FreeRTOSQueueTest, ReceiveOptionalChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueReceive(mock_queue_handle, _, pdMS_TO_TICKS(300)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto result = q.receive(std::chrono::milliseconds(300));
  EXPECT_TRUE(result.has_value());
}

TEST_F(FreeRTOSQueueTest, ReceiveOptionalChronoFail_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueReceive(mock_queue_handle, _, pdMS_TO_TICKS(100)))
      .WillOnce(Return(pdFALSE));
...
TEST_F(FreeRTOSQueueTest, SendIsrNoWoken_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueSendFromISR(mock_queue_handle, _, _))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, SendBackIsrNoWoken_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueSendToBackFromISR(mock_queue_handle, _, _))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, SendFrontIsrNoWoken_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueueSendToFrontFromISR(mock_queue_handle, _, _))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
...
TEST_F(FreeRTOSQueueTest, PeekOptionalFail_BranchCoverage) {
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto result = q.peek(pdMS_TO_TICKS(100));
  EXPECT_FALSE(result.has_value());
}

TEST_F(FreeRTOSQueueTest, PeekOptionalChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueuePeek(mock_queue_handle, _, pdMS_TO_TICKS(200)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto result = q.peek(std::chrono::milliseconds(200));
  EXPECT_TRUE(result.has_value());
}

TEST_F(FreeRTOSQueueTest, PeekOptionalChronoFail_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueuePeek(mock_queue_handle, _, pdMS_TO_TICKS(150)))
      .WillOnce(Return(pdFALSE));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto result = q.peek(std::chrono::milliseconds(150));
  EXPECT_FALSE(result.has_value());
}

TEST_F(FreeRTOSQueueTest, PeekIsrItemNoWoken_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  EXPECT_CALL(*mock, xQueuePeekFromISR(mock_queue_handle, _, _))
      .WillOnce(Return(pdPASS));
...
TEST_F(FreeRTOSQueueTest, SendChronoOverload_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val = 42;
  EXPECT_CALL(*mock, xQueueSend(mock_queue_handle, _, pdMS_TO_TICKS(100)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto rc = q.send(val, std::chrono::milliseconds(100));
  EXPECT_EQ(rc, pdPASS);
}

TEST_F(FreeRTOSQueueTest, SendBackChronoOverload_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val = 42;
  EXPECT_CALL(*mock, xQueueSendToBack(mock_queue_handle, _, pdMS_TO_TICKS(150)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto rc = q.send_back(val, std::chrono::milliseconds(150));
  EXPECT_EQ(rc, pdPASS);
}

TEST_F(FreeRTOSQueueTest, SendFrontChronoOverload_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val = 42;
  EXPECT_CALL(*mock,
              xQueueSendToFront(mock_queue_handle, _, pdMS_TO_TICKS(200)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto rc = q.send_front(val, std::chrono::milliseconds(200));
  EXPECT_EQ(rc, pdPASS);
}

TEST_F(FreeRTOSQueueTest, ReceiveChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val;
  EXPECT_CALL(*mock, xQueueReceive(mock_queue_handle, _, pdMS_TO_TICKS(300)))
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto rc = q.receive(val, std::chrono::milliseconds(300));
  EXPECT_EQ(rc, pdPASS);
}

TEST_F(FreeRTOSQueueTest, PeekChrono_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val;
  EXPECT_CALL(*mock, xQueuePeek(mock_queue_handle, _, pdMS_TO_TICKS(350)))
      .WillOnce(Return(pdPASS));
  EXPECT_CALL(*mock, pcQueueGetName(mock_queue_handle))
      .WillOnce(Return(nullptr));
  EXPECT_CALL(*mock, vQueueDelete(mock_queue_handle));
  ...
  freertos::queue<1, int, freertos::dynamic_queue_allocator<1, int>> q;
  auto rc = q.peek(val, std::chrono::milliseconds(350));
  EXPECT_EQ(rc, pdPASS);
}

TEST_F(FreeRTOSQueueTest, OverwriteIsrNoWokenExisting_BranchCoverage) {
  EXPECT_CALL(*mock, xQueueCreate(1, sizeof(int)))
      .WillOnce(Return(mock_queue_handle));
  int val = 99;
  EXPECT_CALL(*mock, xQueueOverwriteFromISR(mock_queue_handle, _, _))
