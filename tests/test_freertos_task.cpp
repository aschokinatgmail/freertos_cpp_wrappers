 ...
 *
 * Tests cover:
 * - Correct construction, allocation, and deallocation of tasks using static
 * and dynamic allocators
 * - Typical usage patterns for tasks: creation, starting, and deletion
 * - Correct operation of periodic tasks (e.g., tasks that are supposed to run
...
namespace {
// Empty task routines for construction testing - these are never meant to
// execute LCOV_EXCL_START
...
TEST_F(FreeRTOSTaskTest, StaticTaskAllocatorConstruction) {
  ...
  // static_task_allocator<1024> copy_allocator(allocator);  // Should not
  // compile
...
TEST_F(FreeRTOSTaskTest, StaticTaskAllocatorCreate) {
  ...
  TaskHandle_t handle =
      allocator.create(test_task_function, "TestTask", 2, &allocator);
...
TEST_F(FreeRTOSTaskTest, StaticTaskAllocatorCreateNullReturn) {
  ...
  TaskHandle_t handle =
      allocator.create(test_task_function, "TestTask", 2, nullptr);
...
TEST_F(FreeRTOSTaskTest, StaticTaskTerminate) {
  ...
  EXPECT_CALL(*mock, vTaskDelete(mock_task_handle)).Times(1);
  test_task.terminate();

  // After fix: terminate() sets m_hTask = nullptr, so destructor skips
  // vTaskDelete
}
...
TEST_F(FreeRTOSTaskTest, StaticTaskPriority) {
  ...
  EXPECT_CALL(*mock, uxTaskPriorityGet(mock_task_handle)).WillOnce(Return(5));
...
TEST_F(FreeRTOSTaskTest, TaskApplicationTag) {
  ...
  TaskHookFunction_t test_tag =
      reinterpret_cast<TaskHookFunction_t>(0x12345678);
...
TEST_F(FreeRTOSTaskTest, TaskTraceStatus) {
  ...
  EXPECT_CALL(*mock, vTaskGetInfo(mock_task_handle, _, pdFALSE, eInvalid))
      .WillOnce(DoAll(SetArgPointee<1>(expected_status), Return()));
...
TEST_F(FreeRTOSTaskTest, TaskSuspendedOnStart) {
  EXPECT_CALL(*mock,
              xTaskCreateStatic(_, StrEq("SuspendedTask"), _, _, 1, _, _))
      .WillOnce(Return(mock_task_handle));
...
TEST_F(FreeRTOSTaskTest, TaskNotifications) {
  ...
  // Test notify_take
  EXPECT_CALL(*mock, ulTaskNotifyTake(pdTRUE, 100)).WillOnce(Return(42));
  uint32_t value = test_task.notify_take(pdTRUE, 100);
  EXPECT_EQ(value, 42);

  // Test notify
  EXPECT_CALL(*mock,
              xTaskNotify(mock_task_handle, 0x12345678, eSetValueWithOverwrite))
      .WillOnce(Return(pdTRUE));
  ...
  EXPECT_CALL(*mock, xTaskNotifyWait(0, 0xFFFFFFFF, _, 1000))
      .WillOnce(DoAll(SetArgPointee<2>(0xABCDEF00), Return(pdTRUE)));
...
TEST_F(FreeRTOSTaskTest, TaskNotificationsExtended) {
  ...
  uint32_t prev_value;
  EXPECT_CALL(*mock,
              xTaskNotifyAndQuery(mock_task_handle, 0x87654321, eIncrement, _))
      .WillOnce(DoAll(SetArgPointee<3>(0x11111111), Return(pdTRUE)));
  BaseType_t result =
      test_task.notify_and_query(0x87654321, eIncrement, prev_value);
  EXPECT_EQ(result, pdTRUE);
...
TEST_F(FreeRTOSTaskTest, PeriodicTaskConstruction) {
  ...
  // LCOV_EXCL_START - Test lambdas for periodic task callbacks (never execute
  // in unit tests)
  sa::periodic_task<1024> periodic_task("PeriodicTask", 2,
...
TEST_F(FreeRTOSTaskTest, PeriodicTaskWithString) {
  ...
  sa::periodic_task<1024> periodic_task(task_name, 3,
                                        empty_task_routine, // on_start
                                        empty_task_routine, // on_stop
                                        empty_task_routine, // periodic_routine
                                        50ms                // period
...
TEST_F(FreeRTOSTaskTest, PeriodicTaskZeroPeriod) {
  EXPECT_CALL(*mock,
              xTaskCreateStatic(_, StrEq("ZeroPeriodTask"), _, _, 2, _, _))
      .WillOnce(Return(mock_task_handle));
  ...
  sa::periodic_task<1024> periodic_task("ZeroPeriodTask", 2,
                                        empty_task_routine, // on_start
                                        empty_task_routine, // on_stop
                                        empty_task_routine, // periodic_routine
...
TEST_F(FreeRTOSTaskTest, PeriodicTaskIsRunning) {
  ...
  sa::periodic_task<1024> periodic_task("RunningTask", 2,
                                        empty_task_routine, // on_start
                                        empty_task_routine, // on_stop
                                        empty_task_routine, // periodic_routine
                                        100ms               // period
...
TEST_F(FreeRTOSTaskTest, PeriodicTaskTerminate) {
  ...
  sa::periodic_task<1024> periodic_task("TerminateTask", 2,
                                        empty_task_routine, // on_start
                                        empty_task_routine, // on_stop
                                        empty_task_routine, // periodic_routine
                                        100ms               // period
  ...
  EXPECT_CALL(*mock, vTaskDelete(mock_task_handle))
      .Times(1); // Only from terminate(), not from destructor (m_hTask is now
                 // null)
  periodic_task.terminate();
  // abort_delay is skipped because m_hTask is null after terminate()
}
...
TEST_F(FreeRTOSTaskTest, PeriodicTaskNotificationExtensions) {
  EXPECT_CALL(*mock,
              xTaskCreateStatic(_, StrEq("PeriodicNotifyTask"), _, _, 2, _, _))
      .WillOnce(Return(mock_task_handle));
  ...
  sa::periodic_task<1024> periodic_task("PeriodicNotifyTask", 2,
                                        empty_task_routine, // on_start
                                        empty_task_routine, // on_stop
                                        empty_task_routine, // periodic_routine
                                        100ms               // period
  ...
  EXPECT_CALL(*mock, ulTaskNotifyTake(pdTRUE, 1000)).WillOnce(Return(5));
...
TEST_F(FreeRTOSTaskTest, CriticalSection) {
  ...
  // No expectations needed as critical section macros are no-op for host
  // testing
}
...
TEST_F(FreeRTOSTaskTest, CriticalSectionISR) {
  ...
  // No expectations needed as ISR critical section macros are no-op for host
  // testing
}
...
TEST_F(FreeRTOSTaskTest, InterruptBarrier) {
  ...
  // No expectations needed as interrupt control macros are no-op for host
  // testing
}
...
TEST_F(FreeRTOSTaskTest, SchedulerBarrier) {
  ...
  // No expectations needed as scheduler control macros are no-op for host
  // testing
}
...
TEST_F(FreeRTOSTaskTest, StackAllocationLimitation) {
  // This test documents the current limitation that static tasks cannot be
  // allocated on the stack when using the static allocator. This is due to the
  // way FreeRTOS static allocation works.

  /*
   * NOTE: The following code would NOT work correctly in a real FreeRTOS
   * environment:
   *
   * void some_function() {
   *     freertos::sa::task<1024> my_task("StackTask", 1, empty_task_routine);
   * // PROBLEMATIC!
   ...
   *
   * void some_function() {
   *     freertos::da::task<1024> my_task("StackTask", 1, empty_task_routine);
   * // OK
...
TEST_F(FreeRTOSTaskTest, TaskSystemStatus) {
  ...
  EXPECT_CALL(*mock, uxTaskGetSystemState(_, 10, _))
      .WillOnce(DoAll(SetArgPointee<2>(5000), // total runtime
...
TEST_F(FreeRTOSTaskTest, TaskChronoCompatibility) {
  ...
  EXPECT_CALL(*mock, ulTaskNotifyTake(pdTRUE, 500)).WillOnce(Return(1));
  ...
  BaseType_t wait_result =
      test_task.notify_wait(0, 0, notification_value, 1000ms);
  ...
  EXPECT_CALL(*mock,
              xTaskNotifyWait(0xFF, 0xFF00, _, 1)) // 1500us = 1ms (truncated)
      .WillOnce(DoAll(SetArgPointee<2>(0xDEAD), Return(pdTRUE)));
  BaseType_t micro_result =
      test_task.notify_wait(0xFF, 0xFF00, notification_value, 1500us);
...
TEST_F(FreeRTOSTaskTest, TaskMoveConstruction) {
  ...
  // With proper move constructor, only the moved-to object should call
  // destructor
  EXPECT_CALL(*mock, vTaskDelete(mock_task_handle))
  ...
  EXPECT_EQ(original_task.handle(),
            nullptr); // moved-from object should have null handle
}
...
TEST_F(FreeRTOSTaskTest, TaskExecutionDirectCall) {
  ...
  // Use a simple std::function for testing instead of lambda to avoid coverage
  // issues
  std::atomic<bool> task_executed{false};

  // LCOV_EXCL_START - Test lambda never executes in unit test environment
  sa::task<1024> test_task("ExecutionTask", 2,
                           [&task_executed]() { task_executed.store(true); });
  // LCOV_EXCL_STOP
  ...
  EXPECT_FALSE(
      task_executed
          .load()); // Task routine not executed in unit test environment
...
TEST_F(FreeRTOSTaskTest, PeriodicTaskTypo) {
  ...
  uint32_t prev_value;
  EXPECT_CALL(*mock,
              xTaskNotifyAndQuery(mock_task_handle, 0x12345, eSetBits, _))
      .WillOnce(DoAll(SetArgPointee<3>(0xABCD), Return(pdTRUE)));
  BaseType_t result =
      periodic_task.notify_and_query(0x12345, eSetBits, prev_value);
  EXPECT_EQ(result, pdTRUE);
...
TEST_F(FreeRTOSTaskTest, DynamicTaskDifferentSizes) {
  ...
  EXPECT_CALL(*mock, xTaskCreate(_, StrEq("DynTask"), 512, _, 3, _))
      .WillOnce(DoAll(SetArgPointee<5>(mock_handle2), Return(pdPASS)));
...
TEST_F(FreeRTOSTaskTest, TaskNotificationEdgeCases) {
  ...
  EXPECT_CALL(
      *mock, ulTaskNotifyTake(pdTRUE, 1)) // 1500 microseconds = 1ms (truncated)
      .WillOnce(Return(1));
  uint32_t result =
      test_task.notify_take(pdTRUE, std::chrono::microseconds(1500));
  EXPECT_EQ(result, 1);

  // Test notification wait with different duration types
  uint32_t notify_val;
  EXPECT_CALL(*mock, xTaskNotifyWait(0, 0, _, 5000)) // 5 seconds = 5000ms
      .WillOnce(DoAll(SetArgPointee<2>(0x12345), Return(pdTRUE)));
  BaseType_t wait_result =
      test_task.notify_wait(0, 0, notify_val, std::chrono::seconds(5));
  EXPECT_EQ(wait_result, pdTRUE);
...
TEST_F(FreeRTOSTaskTest, TaskSystemStatusTemplate) {
  ...
  EXPECT_CALL(*mock, uxTaskGetSystemState(_, 10, _))
      .WillOnce(DoAll(SetArgPointee<2>(12345), Return(5)));
...
TEST_F(FreeRTOSTaskTest, DelayFunctionsWithDifferentDurations) {
  ...
  EXPECT_CALL(*mock, vTaskDelay(pdMS_TO_TICKS(2000))).Times(1);
  freertos::delay(std::chrono::seconds(2));

  EXPECT_CALL(*mock, vTaskDelay(pdMS_TO_TICKS(1500))).Times(1);
  freertos::delay(std::chrono::milliseconds(1500));

  EXPECT_CALL(*mock, vTaskDelay(pdMS_TO_TICKS(500))).Times(1);
...
TEST_F(FreeRTOSTaskTest, RacingConditionTaskConstructorInitialization) {
  // Test the racing condition fix in constructor where member initialization
  // order matters m_hTask must be initialized after m_taskRoutine to prevent
  // race condition
  ...
  EXPECT_CALL(*mock, xTaskCreateStatic(_, _, _, _, _, _, _))
      .WillOnce([&routine_set_before_handle,
                 this](auto task_func, auto name, auto stack_size, auto params,
                       auto priority, auto stack, auto tcb) {
...
TEST_F(FreeRTOSTaskTest, ConcurrentTaskCreationAndDestruction) {
  ...
  TaskHandle_t handles[3] = {reinterpret_cast<TaskHandle_t>(0x1001),
...
TEST_F(FreeRTOSTaskTest, PeriodicTaskLifecycleRacingConditions) {
  ...
  sa::periodic_task<1024> periodic_task("PeriodicRace", 2,
                                        empty_task_routine, // on_start
                                        empty_task_routine, // on_stop
                                        empty_task_routine, // periodic_routine
  ...
  EXPECT_CALL(*mock, vTaskResume(mock_task_handle)).Times(AtLeast(1));
  periodic_task.resume(); // Resume task

  EXPECT_CALL(*mock, vTaskSuspend(mock_task_handle)).Times(AtLeast(1));
...
TEST_F(FreeRTOSTaskTest, NotificationRacingConditions) {
  ...
  for (int i = 0; i < 5; ++i) {
    EXPECT_CALL(*mock,
                xTaskNotifyAndQuery(mock_task_handle, i + 1, eSetBits, _))
        .WillOnce(DoAll(SetArgPointee<3>(i * 10), Return(pdTRUE)));

    BaseType_t result =
        test_task.notify_and_query(i + 1, eSetBits, prev_values[i]);
    EXPECT_EQ(result, pdTRUE);
...
TEST_F(FreeRTOSTaskTest, ComplexMultitaskingScenario) {
  ...
  EXPECT_CALL(*mock, uxTaskPriorityGet(producer_handle)).WillOnce(Return(3));
  UBaseType_t prod_priority = producer.priority();
  EXPECT_EQ(prod_priority, 3);

  EXPECT_CALL(*mock, uxTaskPriorityGet(consumer_handle)).WillOnce(Return(2));
  ...
  EXPECT_CALL(*mock, eTaskGetState(producer_handle)).WillOnce(Return(eRunning));
  eTaskState prod_state = producer.state();
  EXPECT_EQ(prod_state, eRunning);

  EXPECT_CALL(*mock, eTaskGetState(consumer_handle)).WillOnce(Return(eBlocked));
...
TEST_F(FreeRTOSTaskTest, TaskSystemStatusUnderLoad) {
  ...
  TaskHandle_t handles[3] = {reinterpret_cast<TaskHandle_t>(0x4001),
  ...
  EXPECT_CALL(*mock, uxTaskGetSystemState(_, 10, _))
      .WillOnce(DoAll(SetArgPointee<2>(15000), // Total runtime
                      Return(3)                // Number of tasks
                      ));
...
TEST_F(FreeRTOSTaskTest, ConstructorInitializationOrderRaceCondition) {
  ...
  // where m_hTask initialization was moved to be last to prevent race
  // conditions

  std::atomic<bool> task_routine_called{false};
  std::atomic<bool> task_created_callback_called{false};

  // This lambda will be called during xTaskCreateStatic - simulating task
  // immediate execution
  EXPECT_CALL(*mock, xTaskCreateStatic(_, _, _, _, _, _, _))
      .WillOnce([&](auto task_func, auto name, auto stack_size, auto params,
        ...
        // Verify that the task object pointer is valid and members are
        // initialized
        EXPECT_NE(task_ptr, nullptr);

        // The fix ensures m_taskRoutine is set before m_hTask is created
        // This prevents the race condition where task could start before
        // routine is set
  ...
  // LCOV_EXCL_START - Test lambda never executes in unit test environment
  sa::task<1024> race_test_task(
      "RaceTest", 2, [&task_routine_called]() { task_routine_called = true; });
  // LCOV_EXCL_STOP
...
TEST_F(FreeRTOSTaskTest, TaskExecutionInternalFunction) {
  ...
  sa::task<1024> suspended_task("SuspendedTask", 1, empty_task_routine,
  ...
  sa::task<1024> normal_task("NormalTask", 2,
                             empty_task_routine); // start_suspended defaults to
                                                  // false for this constructor
...
TEST_F(FreeRTOSTaskTest, PeriodicTaskRunMethodExecution) {
  ...
  EXPECT_CALL(*mock,
              xTaskCreateStatic(_, StrEq("PeriodicRunTask"), _, _, 1, _, _))
      .WillOnce(Return(mock_task_handle));
...
TEST_F(FreeRTOSTaskTest, TaskSystemStatusCoverage) {
  ...
  EXPECT_CALL(*mock, uxTaskGetSystemState(_, 10, _))
      .WillOnce(DoAll(SetArgPointee<2>(50000), // total runtime
  ...
  size_t count = 0;
  for (auto iter = status.begin(); iter != status.end() && count < 5;
       ++iter, ++count) {
...
TEST_F(FreeRTOSTaskTest, EdgeCaseErrorHandling) {
  ...
  EXPECT_CALL(*mock, uxTaskPriorityGet(nullptr)).WillOnce(Return(0));
...
TEST_F(FreeRTOSTaskTest, AdvancedChronoCompatibility) {
  ...
  EXPECT_CALL(*mock,
              ulTaskNotifyTake(pdTRUE, 0)) // 100 microseconds rounds to 0
      .WillOnce(Return(0));
  uint32_t result =
      test_task.notify_take(pdTRUE, std::chrono::microseconds(100));
  EXPECT_EQ(result, 0);
  ...
  EXPECT_CALL(*mock, xTaskNotifyWait(0xFFFF, 0x0000, _, 1500)) // 1.5 seconds
      .WillOnce(DoAll(SetArgPointee<2>(0x5555), Return(pdTRUE)));

  using namespace std::chrono_literals;
  auto duration = 1500ms;
  BaseType_t wait_result =
      test_task.notify_wait(0xFFFF, 0x0000, notify_val, duration);
  EXPECT_EQ(wait_result, pdTRUE);
...
TEST_F(FreeRTOSTaskTest, PriorityInheritanceScenario) {
  ...
  EXPECT_CALL(*mock, uxTaskPriorityGet(low_prio_handle)).WillOnce(Return(5));
