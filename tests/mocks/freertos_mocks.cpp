...
std::shared_ptr<::testing::StrictMock<::FreeRTOSMock>>
    FreeRTOSMockInstance::instance_;
...
TaskHandle_t xTaskCreateStatic(TaskFunction_t pxTaskCode, const char *pcName,
  ...
  if (g_freertos_mock) {
    return g_freertos_mock->xTaskCreateStatic(pxTaskCode, pcName, ulStackDepth,
...
BaseType_t xTaskCreate(TaskFunction_t pxTaskCode, const char *pcName,
  ...
  if (g_freertos_mock) {
    return g_freertos_mock->xTaskCreate(pxTaskCode, pcName, usStackDepth,
                                        pvParameters, uxPriority,
                                        pxCreatedTask);
  }
  return pdFAIL;
...
BaseType_t xTaskNotifyAndQuery(TaskHandle_t xTaskToNotify, uint32_t ulValue,
  ...
  if (g_freertos_mock) {
    return g_freertos_mock->xTaskNotifyAndQuery(xTaskToNotify, ulValue, eAction,
...
BaseType_t xTaskNotifyFromISR(TaskHandle_t xTaskToNotify, uint32_t ulValue,
  ...
  if (g_freertos_mock) {
    return g_freertos_mock->xTaskNotifyFromISR(xTaskToNotify, ulValue, eAction,
...
BaseType_t xTaskNotifyAndQueryFromISR(TaskHandle_t xTaskToNotify,
                                      ...
                                      BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xTaskNotifyAndQueryFromISR(
...
BaseType_t xTaskNotifyWait(uint32_t ulBitsToClearOnEntry,
                           ...
                           TickType_t xTicksToWait) {
  if (g_freertos_mock) {
    return g_freertos_mock->xTaskNotifyWait(ulBitsToClearOnEntry,
                                            ulBitsToClearOnExit,
...
void vTaskGetInfo(TaskHandle_t xTask, TaskStatus_t *pxTaskStatus,
  ...
  if (g_freertos_mock) {
    g_freertos_mock->vTaskGetInfo(xTask, pxTaskStatus, xGetFreeStackSpace,
                                  eState);
  }
}
...
UBaseType_t uxTaskGetSystemState(TaskStatus_t *pxTaskStatusArray,
  ...
  if (g_freertos_mock) {
    return g_freertos_mock->uxTaskGetSystemState(pxTaskStatusArray, uxArraySize,
                                                 pulTotalRunTime);
  }
  return 0;
}
...
SemaphoreHandle_t xSemaphoreCreateCounting(UBaseType_t uxMaxCount,
  ...
  if (g_freertos_mock) {
    return g_freertos_mock->xSemaphoreCreateCounting(uxMaxCount,
                                                     uxInitialCount);
  }
  return nullptr;
}
...
xSemaphoreCreateCountingStatic(UBaseType_t uxMaxCount,
  ...
  if (g_freertos_mock) {
    return g_freertos_mock->xSemaphoreCreateCountingStatic(
        uxMaxCount, uxInitialCount, pxSemaphoreBuffer);
  }
  return nullptr;
}
...
BaseType_t xSemaphoreGiveFromISR(SemaphoreHandle_t xSemaphore,
                                 BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xSemaphoreGiveFromISR(xSemaphore,
                                                  pxHigherPriorityTaskWoken);
  }
  return pdFALSE;
}
...
BaseType_t xSemaphoreTakeFromISR(SemaphoreHandle_t xSemaphore,
                                 BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xSemaphoreTakeFromISR(xSemaphore,
                                                  pxHigherPriorityTaskWoken);
  }
  return pdFALSE;
}
...
QueueHandle_t xQueueCreateStatic(UBaseType_t uxQueueLength,
  ...
  if (g_freertos_mock) {
    return g_freertos_mock->xQueueCreateStatic(uxQueueLength, uxItemSize,
                                               pucQueueStorage, pxStaticQueue);
  }
  return nullptr;
}
...
BaseType_t xQueueSendToBack(QueueHandle_t xQueue, const void *pvItemToQueue,
                            TickType_t xTicksToWait) {
  if (g_freertos_mock) {
    return g_freertos_mock->xQueueSendToBack(xQueue, pvItemToQueue,
                                             xTicksToWait);
  }
  return pdFAIL;
}
...
BaseType_t xQueueSendToFront(QueueHandle_t xQueue, const void *pvItemToQueue,
                             TickType_t xTicksToWait) {
  if (g_freertos_mock) {
    return g_freertos_mock->xQueueSendToFront(xQueue, pvItemToQueue,
                                              xTicksToWait);
  }
  return pdFAIL;
}
...
BaseType_t xQueueSendFromISR(QueueHandle_t xQueue, const void *pvItemToQueue,
                             BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xQueueSendFromISR(xQueue, pvItemToQueue,
                                              pxHigherPriorityTaskWoken);
  }
  return pdFAIL;
}
...
BaseType_t xQueueSendToBackFromISR(QueueHandle_t xQueue,
                                   ...
                                   BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xQueueSendToBackFromISR(xQueue, pvItemToQueue,
                                                    pxHigherPriorityTaskWoken);
  }
  return pdFAIL;
}
...
BaseType_t xQueueSendToFrontFromISR(QueueHandle_t xQueue,
                                    ...
                                    BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xQueueSendToFrontFromISR(xQueue, pvItemToQueue,
                                                     pxHigherPriorityTaskWoken);
  }
  return pdFAIL;
}
...
BaseType_t xQueueReceiveFromISR(QueueHandle_t xQueue, void *pvBuffer,
                                BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xQueueReceiveFromISR(xQueue, pvBuffer,
                                                 pxHigherPriorityTaskWoken);
  }
  return pdFAIL;
}
...
BaseType_t xQueuePeekFromISR(QueueHandle_t xQueue, void *pvBuffer,
                             BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xQueuePeekFromISR(xQueue, pvBuffer,
                                              pxHigherPriorityTaskWoken);
  }
  return pdFAIL;
}
...
BaseType_t xQueueOverwriteFromISR(QueueHandle_t xQueue,
                                  ...
                                  BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xQueueOverwriteFromISR(xQueue, pvItemToQueue,
                                                   pxHigherPriorityTaskWoken);
  }
  return pdPASS;
}
...
EventBits_t xEventGroupSetBitsFromISR(EventGroupHandle_t xEventGroup,
                                      ...
                                      BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xEventGroupSetBitsFromISR(
...
EventBits_t xEventGroupWaitBits(EventGroupHandle_t xEventGroup,
                                ...
                                TickType_t xTicksToWait) {
  if (g_freertos_mock) {
    return g_freertos_mock->xEventGroupWaitBits(xEventGroup, uxBitsToWaitFor,
...
EventBits_t xEventGroupSync(EventGroupHandle_t xEventGroup,
                            ...
                            TickType_t xTicksToWait) {
  if (g_freertos_mock) {
    return g_freertos_mock->xEventGroupSync(xEventGroup, uxBitsToSet,
                                            uxBitsToWaitFor, xTicksToWait);
  }
  return 0;
}
...
TimerHandle_t xTimerCreate(const char *pcTimerName,
  ...
  auto &enhanced_mock = freertos_mocks::EnhancedTimerMock::instance();
  if (enhanced_mock.isSimulationEnabled()) {
    return enhanced_mock.getSimulator().createTimer(
        pcTimerName, xTimerPeriodInTicks, uxAutoReload, pvTimerID,
        pxCallbackFunction, false);
  }

  if (g_freertos_mock) {
    return g_freertos_mock->xTimerCreate(pcTimerName, xTimerPeriodInTicks,
...
TimerHandle_t xTimerCreateStatic(const char *pcTimerName,
  ...
  if (enhanced_mock.isSimulationEnabled()) {
    return enhanced_mock.getSimulator().createTimer(
        pcTimerName, xTimerPeriodInTicks, uxAutoReload, pvTimerID,
        pxCallbackFunction, true);
  }

  if (g_freertos_mock) {
    return g_freertos_mock->xTimerCreateStatic(
        pcTimerName, xTimerPeriodInTicks, uxAutoReload, pvTimerID,
...
BaseType_t xTimerStartFromISR(TimerHandle_t xTimer,
  ...
  auto &enhanced_mock = freertos_mocks::EnhancedTimerMock::instance();
  if (enhanced_mock.isSimulationEnabled()) {
    return enhanced_mock.getSimulator().startTimerFromISR(
        xTimer, pxHigherPriorityTaskWoken);
  }

  if (g_freertos_mock) {
    return g_freertos_mock->xTimerStartFromISR(xTimer,
                                               pxHigherPriorityTaskWoken);
  }
  return pdFAIL;
}
...
BaseType_t xTimerStopFromISR(TimerHandle_t xTimer,
  ...
  auto &enhanced_mock = freertos_mocks::EnhancedTimerMock::instance();
  if (enhanced_mock.isSimulationEnabled()) {
    return enhanced_mock.getSimulator().stopTimerFromISR(
        xTimer, pxHigherPriorityTaskWoken);
  }

  if (g_freertos_mock) {
    return g_freertos_mock->xTimerStopFromISR(xTimer,
                                              pxHigherPriorityTaskWoken);
  }
  return pdFAIL;
}
...
BaseType_t xTimerResetFromISR(TimerHandle_t xTimer,
  ...
  auto &enhanced_mock = freertos_mocks::EnhancedTimerMock::instance();
  if (enhanced_mock.isSimulationEnabled()) {
    return enhanced_mock.getSimulator().resetTimerFromISR(
        xTimer, pxHigherPriorityTaskWoken);
  }

  if (g_freertos_mock) {
    return g_freertos_mock->xTimerResetFromISR(xTimer,
                                               pxHigherPriorityTaskWoken);
  }
  return pdFAIL;
}
...
BaseType_t xTimerChangePeriod(TimerHandle_t xTimer, TickType_t xNewPeriod,
  ...
  auto &enhanced_mock = freertos_mocks::EnhancedTimerMock::instance();
  if (enhanced_mock.isSimulationEnabled()) {
    return enhanced_mock.getSimulator().changePeriod(xTimer, xNewPeriod,
                                                     xTicksToWait);
  }

  if (g_freertos_mock) {
    return g_freertos_mock->xTimerChangePeriod(xTimer, xNewPeriod,
                                               xTicksToWait);
  }
  return pdFAIL;
}
...
BaseType_t xTimerChangePeriodFromISR(TimerHandle_t xTimer,
  ...
  auto &enhanced_mock = freertos_mocks::EnhancedTimerMock::instance();
  if (enhanced_mock.isSimulationEnabled()) {
    return enhanced_mock.getSimulator().changePeriodFromISR(
        xTimer, xNewPeriod, pxHigherPriorityTaskWoken);
  }

  if (g_freertos_mock) {
    return g_freertos_mock->xTimerChangePeriodFromISR(
        xTimer, xNewPeriod, pxHigherPriorityTaskWoken);
  }
  return pdFAIL;
}
...
void vTimerSetTimerID(TimerHandle_t xTimer, void *pvNewTimerID) {
  if (g_freertos_mock) {
    g_freertos_mock->vTimerSetTimerID(xTimer, pvNewTimerID);
  }
}
...
xMessageBufferCreateStatic(size_t xBufferSizeBytes,
                           uint8_t *pucMessageBufferStorageArea,
                           StaticMessageBuffer_t *pxStaticMessageBuffer) {
  if (g_freertos_mock) {
    return g_freertos_mock->xMessageBufferCreateStatic(
...
size_t xMessageBufferSend(MessageBufferHandle_t xMessageBuffer,
                          ...
                          TickType_t xTicksToWait) {
  if (g_freertos_mock) {
    return g_freertos_mock->xMessageBufferSend(xMessageBuffer, pvTxData,
...
size_t xMessageBufferSendFromISR(MessageBufferHandle_t xMessageBuffer,
                                 ...
                                 BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xMessageBufferSendFromISR(
        xMessageBuffer, pvTxData, xDataLengthBytes, pxHigherPriorityTaskWoken);
  }
  return 0;
}
...
size_t xMessageBufferReceive(MessageBufferHandle_t xMessageBuffer,
                             ...
                             TickType_t xTicksToWait) {
  if (g_freertos_mock) {
    return g_freertos_mock->xMessageBufferReceive(
        xMessageBuffer, pvRxData, xBufferLengthBytes, xTicksToWait);
  }
  return 0;
}
...
size_t xMessageBufferReceiveFromISR(MessageBufferHandle_t xMessageBuffer,
                                    ...
                                    BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xMessageBufferReceiveFromISR(
        xMessageBuffer, pvRxData, xBufferLengthBytes,
        pxHigherPriorityTaskWoken);
  }
  return 0;
}
...
StreamBufferHandle_t xStreamBufferCreate(size_t xBufferSizeBytes,
  ...
  if (g_freertos_mock) {
    return g_freertos_mock->xStreamBufferCreate(xBufferSizeBytes,
                                                xTriggerLevelBytes);
  }
  return nullptr;
}
...
StreamBufferHandle_t
xStreamBufferCreateStatic(size_t xBufferSizeBytes, size_t xTriggerLevelBytes,
                           uint8_t *pucStreamBufferStorageArea,
                           StaticStreamBuffer_t *pxStaticStreamBuffer) {
  if (g_freertos_mock) {
    return g_freertos_mock->xStreamBufferCreateStatic(
...
xStreamBufferGenericCreateStatic(size_t xBufferSizeBytes, size_t xTriggerLevelBytes,
                                  ...
                                  StaticStreamBuffer_t *pxStaticStreamBuffer) {
  if (g_freertos_mock) {
    return g_freertos_mock->xStreamBufferGenericCreateStatic(
...
size_t xStreamBufferSend(StreamBufferHandle_t xStreamBuffer,
                         ...
                         TickType_t xTicksToWait) {
  if (g_freertos_mock) {
    return g_freertos_mock->xStreamBufferSend(xStreamBuffer, pvTxData,
                                              xDataLengthBytes, xTicksToWait);
  }
  return 0;
}
...
size_t xStreamBufferSendFromISR(StreamBufferHandle_t xStreamBuffer,
                                ...
                                BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xStreamBufferSendFromISR(
...
size_t xStreamBufferReceive(StreamBufferHandle_t xStreamBuffer, void *pvRxData,
                            ...
                            TickType_t xTicksToWait) {
  if (g_freertos_mock) {
    return g_freertos_mock->xStreamBufferReceive(
        xStreamBuffer, pvRxData, xBufferLengthBytes, xTicksToWait);
  }
  return 0;
}
...
size_t xStreamBufferReceiveFromISR(StreamBufferHandle_t xStreamBuffer,
                                   ...
                                   BaseType_t *pxHigherPriorityTaskWoken) {
  if (g_freertos_mock) {
    return g_freertos_mock->xStreamBufferReceiveFromISR(
...
StreamBufferHandle_t xStreamBufferGenericCreateWithCallback(
    ...
    void *pvReceiveCompletedCallbackContext) {
  if (g_freertos_mock) {
    return g_freertos_mock->xStreamBufferGenericCreateWithCallback(
        xBufferSizeBytes, xTriggerLevelBytes, xIsMessageBuffer,
...
StreamBufferHandle_t xStreamBufferGenericCreateStaticWithCallback(
    ...
    void *pvReceiveCompletedCallbackContext) {
  if (g_freertos_mock) {
    return g_freertos_mock->xStreamBufferGenericCreateStaticWithCallback(
        xBufferSizeBytes, xTriggerLevelBytes, xIsMessageBuffer,
        pucStreamBufferStorageArea, pxStaticStreamBuffer,
        pxSendCompletedCallback, pvSendCompletedCallbackContext,
        pxReceiveCompletedCallback, pvReceiveCompletedCallbackContext);
